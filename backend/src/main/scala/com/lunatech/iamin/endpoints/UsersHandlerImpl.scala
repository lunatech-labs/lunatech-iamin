...
class UsersHandlerImpl[F[_] : Async](service: UserService[F], obs: IdObfuscator) extends UsersHandler[F] {
  ...
  override def getUsers(respond: GetUsersResponse.type)
                       (maybeObfuscatedOffsetId: Option[String] = None, maybeLimit: Option[Int] = Option(10))
  : F[GetUsersResponse] = {
  ...
  override def getUsersById(respond: GetUsersByIdResponse.type)(obfuscatedUserId: String): F[GetUsersByIdResponse] = {
    ...
    service.get(userId).fold[GetUsersByIdResponse](
  ...
  override def patchUserById(respond: PatchUserByIdResponse.type)
                            (obfuscatedUserId: String, body: PatchUserRequestJson): F[PatchUserByIdResponse] = {
    val userId = obs deobfuscate obfuscatedUserId
    service.update(userId, body.displayName).fold[PatchUserByIdResponse](
      respond.NotFound(problemOf(Status.NotFound, s"User '$obfuscatedUserId' was not found"))
    )(
      respond.Ok(_)
    )
  ...
  override def deleteUserById(respond: DeleteUserByIdResponse.type)
                             (obfuscatedUserId: String): F[DeleteUserByIdResponse] = {
    val userId = obs deobfuscate obfuscatedUserId
    service.delete(userId).fold[DeleteUserByIdResponse](
      respond.NotFound(problemOf(Status.NotFound, s"User '$obfuscatedUserId' was not found"))
    )(
      _ => respond.NoContent
    )
